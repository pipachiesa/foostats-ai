...
# filterpy is an optional dependency. The Kalman-smoothing arc removal in
# remove_arcs_from_path() needs it, but it is not part of the foostats_ai venv.
# When it is absent we keep the rest of the cleanup pipeline working
# (lone-point / nearby filtering + gap filling) and simply skip arc removal.
try:
...
def remove_arcs_from_path(coordinates):
    ...
    #
    # Adapted: returns None when filterpy is unavailable so the caller
    # (Ball.remove_arcs) can skip arc removal without crashing.
    if not HAS_FILTERPY:
        return None

    # The RANSAC segmentation below slices traj[500:], so this radar-style arc
    # removal is only meaningful on long sequences (full matches). On short
    # clips it cannot run — skip rather than crash. Real broadcast footage has
    # thousands of frames, so this only affects tiny demo/test inputs.
